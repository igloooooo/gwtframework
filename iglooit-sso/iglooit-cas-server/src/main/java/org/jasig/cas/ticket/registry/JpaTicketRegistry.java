...
/**
 * Copied from CAS and changed the @PersistenceContext annotation to fix the ambiguous entity manager bean definition.
...
{
    @NotNull
    @PersistenceContext(unitName = "CasPersistence")
    ...
    {
        return countToInt(
            entityManager.createQuery("select count(t) from TicketGrantingTicketImpl t").getSingleResult());
    }
